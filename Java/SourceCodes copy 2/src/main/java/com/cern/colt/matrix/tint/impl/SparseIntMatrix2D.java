...
import com.cern.colt.map.tlong.AbstractLongIntMap;
import com.cern.colt.map.tlong.OpenLongIntHashMap;
 ...
 * {@link com.cern.colt.map.tint.OpenIntIntHashMap}, which is a compact and
 ...
 * @see com.cern.colt.map.tint.OpenIntIntHashMap
...
public class SparseIntMatrix2D extends IntMatrix2D {
     ...
     * to memory usage see {@link OpenLongIntHashMap}.
     * 
     * @param rows
     *            the number of rows the matrix shall have.
     * @param columns
    ...
    public IntMatrix2D assign(com.cern.colt.function.tint.IntFunction function) {
        if (this.isNoView && function instanceof com.cern.jet.math.tint.IntMult) { // x[i] = mult*x[i]
    ...
    public IntMatrix2D assign(final IntMatrix2D y, com.cern.colt.function.tint.IntIntFunction function) {
        ...
        } else if (function == com.cern.jet.math.tint.IntFunctions.mult) { // x[i] = x[i] * y[i]
            this.elements.forEachPair(new com.cern.colt.function.tlong.LongIntProcedure() {
        ...
        } else if (function == com.cern.jet.math.tint.IntFunctions.div) { // x[i] = x[i] / y[i]
            this.elements.forEachPair(new com.cern.colt.function.tlong.LongIntProcedure() {
                public boolean apply(long key, int value) {
                    int i = (int) (key / columns);
                    int j = (int) (key % columns);
    ...
    public SparseIntMatrix2D assign(final int[] rowIndexes, final int[] columnIndexes, final int value,
            final com.cern.colt.function.tint.IntIntFunction function) {
        int size = rowIndexes.length;
        if (function == com.cern.jet.math.tint.IntFunctions.plus) { // x[i] = x[i] + y[i]
