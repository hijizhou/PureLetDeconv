...
import com.cern.colt.matrix.Former;
 ...
 * <tt> float[][] values = {<br>
 ...
 com.hep.aida.bin.BinFunctions1D F = com.hep.aida.bin.BinFunctions1D.functions; // alias<br>
 com.hep.aida.bin.BinFunction1D[] aggr = {F.mean, F.rms, F.quantile(0.25), F.median, F.quantile(0.75), F.stdDev, F.min, F.max};<br>
 ...
 * passed as format string, {@link Float#toString(float)} is used
...
public class FloatFormatter extends AbstractFormatter {
    ...
    public static void demo1() {
        ...
        FloatMatrix2D matrix = com.cern.colt.matrix.tfloat.FloatFactory2D.dense.make(values);
    ...
    public static void demo3(int size, float value) {
        com.cern.colt.Timer timer = new com.cern.colt.Timer();
        String s;
        StringBuffer buf;
        FloatMatrix2D matrix = com.cern.colt.matrix.tfloat.FloatFactory2D.dense.make(size, size, value);
        ...
        timer.reset().start();
        Former format = new com.cern.colt.matrix.FormerFactory().create("%G");
        buf = new StringBuffer();
        for (int i = size; --i >= 0;) {
            for (int j = size; --j >= 0;) {
    ...
    public static void demo5() {
        ...
        System.out.println(com.cern.colt.matrix.tfloat.FloatFactory2D.dense.make(values));
        System.out.println(new FloatFormatter("%G").toTitleString(com.cern.colt.matrix.tfloat.FloatFactory2D.dense
    ...
    public static void demo6() {
        ...
        // System.out.println(com.cern.colt.matrix.FloatFactory2D.dense.make(values));
        // System.out.println(new
        // Formatter().toSourceCode(com.cern.colt.matrix.FloatFactory2D.dense.make(values)));
        System.out.println(new FloatFormatter().toString(com.cern.colt.matrix.tfloat.FloatFactory2D.dense.make(values)));
        System.out.println(new FloatFormatter().toTitleString(
                com.cern.colt.matrix.tfloat.FloatFactory2D.dense.make(values), rowNames, columnNames, "vendor", "density",
                "title"));
    }
    ...
    public static void demo7() {
        ...
        com.hep.aida.tfloat.bin.FloatBinFunctions1D F = com.hep.aida.tfloat.bin.FloatBinFunctions1D.functions;
        com.hep.aida.tfloat.bin.FloatBinFunction1D[] aggr = { F.mean, F.rms, F.quantile(0.25f), F.median,
                F.quantile(0.75f), F.stdDev, F.min, F.max };
        String format = "%1.2G";
        System.out.println(new FloatFormatter(format).toTitleString(com.cern.colt.matrix.tfloat.FloatFactory2D.dense
    ...
    protected String toTitleString(FloatMatrix2D matrix, String[] rowNames, String[] columnNames, String rowAxisName,
        ...
        return new com.cern.colt.matrix.tobject.algo.ObjectFormatter().toTitleString(
                com.cern.colt.matrix.tobject.ObjectFactory2D.dense.make(s), rowNames, columnNames, rowAxisName,
    ...
    public String toTitleString(FloatMatrix2D matrix, String[] rowNames, String[] columnNames, String rowAxisName,
            String columnAxisName, String title, com.hep.aida.tfloat.bin.FloatBinFunction1D[] aggr) {
        if (matrix.size() == 0)
            return "Empty matrix";
        ...
        FloatStatistic.aggregate(matrix, aggr, colStats); // aggregate
        // an
        // entire
        // column
        // at a
        // time
        FloatStatistic.aggregate(matrix.viewDice(), aggr, rowStats.viewDice()); // aggregate
        // an
        // entire
        // row
        // at a
        ...
        // aggregations
        com.cern.colt.matrix.tobject.ObjectMatrix2D allStats = com.cern.colt.matrix.tobject.ObjectFactory2D.dense.make(matrix
                .rows()
        ...
        if (columnNames != null) {
            com.cern.colt.list.tobject.ObjectArrayList list = new com.cern.colt.list.tobject.ObjectArrayList(columnNames);
            list.add("|");
        ...
        if (rowNames != null) {
            com.cern.colt.list.tobject.ObjectArrayList list = new com.cern.colt.list.tobject.ObjectArrayList(rowNames);
            for (int i = 0; i < aggr.length; i++)
                list.add(aggr[i].name()); // add names of aggregation
            // functions
        ...
        String s = new com.cern.colt.matrix.tobject.algo.ObjectFormatter().toTitleString(allStats, rowNames, columnNames,
    ...
    public String toTitleString(FloatMatrix3D matrix, String[] sliceNames, String[] rowNames, String[] columnNames,
            ...
            com.hep.aida.tfloat.bin.FloatBinFunction1D[] aggr) {
        if (matrix.size() == 0)
            return "Empty matrix";
        StringBuffer buf = new StringBuffer();
