...
public class DenseIntMatrix1D extends IntMatrix1D {
    ...
    public IntMatrix1D assign(final com.cern.colt.function.tint.IntFunction function) {
        final int multiplicator;
        if (function instanceof com.cern.jet.math.tint.IntMult) {
            // x[i] = mult*x[i]
            multiplicator = ((com.cern.jet.math.tint.IntMult) function).multiplicator;
            if (multiplicator == 1) {
                return this;
            }
        } else {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        } else if (function instanceof com.cern.jet.math.tint.IntPlusMultFirst) {
                            int multiplicator = ((com.cern.jet.math.tint.IntPlusMultFirst) function).multiplicator;
                            if (multiplicator == 0) {
                                // x[i] = 0*x[i] + y[i]
                                for (int k = firstIdx; k < lastIdx; k++) {
                                    elements[idx] = elemsOther[idxOther];
