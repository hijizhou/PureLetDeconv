...
import com.cern.jet.math.tint.IntFunctions;
 ...
 * {@link IntFunctions#plusMultSecond} as argument.
 * 
 * 
...
public class SparseRCIntMatrix2D extends WrapperIntMatrix2D {
    ...
    public IntMatrix2D assign(final IntMatrix2D y, com.cern.colt.function.tint.IntIntFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCIntMatrix2D) && (function == IntFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            if (function == IntFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == IntFunctions.mult) { // x[i] = x[i] * y[i]
            for (int i = rows; --i >= 0;) {
        ...
        if (function == IntFunctions.div) { // x[i] = x[i] / y[i]
