 ...
 * 
 */
public class LongSorting extends com.cern.colt.PersistentObject {
    /**
     * 
     */
     ...
     */
    public LongMatrix1D sort(final LongMatrix1D vector, final com.cern.colt.function.tlong.LongComparator c) {
     ...
     * @return sorted indexes
     */
    public int[] sortIndex(final LongMatrix1D vector, final com.cern.colt.function.tlong.LongComparator c) {
        int[] indexes = new int[(int) vector.size()]; // row indexes to reorder
        // instead of matrix itself
        for (int i = indexes.length; --i >= 0;)
            indexes[i] = i;
