...
import com.cern.colt.matrix.tdouble.impl.DenseColumnDoubleMatrix2D;
...
public class DenseColumnDComplexMatrix2D extends DComplexMatrix2D {
    ...
    public double[] aggregate(final com.cern.colt.function.tdcomplex.DComplexDComplexDComplexFunction aggr,
            final com.cern.colt.function.tdcomplex.DComplexDComplexFunction f) {
    ...
    public double[] aggregate(final DComplexMatrix2D other,
            final com.cern.colt.function.tdcomplex.DComplexDComplexDComplexFunction aggr,
            final com.cern.colt.function.tdcomplex.DComplexDComplexDComplexFunction f) {
    ...
    public DComplexMatrix2D assign(final com.cern.colt.function.tdcomplex.DComplexDComplexFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tdcomplex.DComplexMult) {
                double[] multiplicator = ((com.cern.jet.math.tdcomplex.DComplexMult) function).multiplicator;
    ...
    public DComplexMatrix2D assign(final com.cern.colt.function.tdcomplex.DComplexProcedure cond,
            final com.cern.colt.function.tdcomplex.DComplexDComplexFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
    ...
    public DComplexMatrix2D assign(final com.cern.colt.function.tdcomplex.DComplexProcedure cond, final double[] value) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
    ...
    public DComplexMatrix2D assign(final com.cern.colt.function.tdcomplex.DComplexRealFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        int idx = zero + firstColumn * columnStride;
                        double[] tmp = new double[2];
                        if (function == com.cern.jet.math.tdcomplex.DComplexFunctions.abs) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, r = 0; r < rows; r++) {
                                    tmp[0] = elements[i];
                                    tmp[1] = elements[i + 1];
    ...
    public DComplexMatrix2D assign(final DComplexMatrix2D y,
            final com.cern.colt.function.tdcomplex.DComplexDComplexDComplexFunction function) {
        // overriden for performance only
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        if (function == com.cern.jet.math.tdcomplex.DComplexFunctions.mult) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
                        ...
                        } else if (function == com.cern.jet.math.tdcomplex.DComplexFunctions.multConjFirst) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
                        ...
                        } else if (function == com.cern.jet.math.tdcomplex.DComplexFunctions.multConjSecond) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
