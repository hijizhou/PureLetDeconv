...
import com.org.jtransforms.dct.DoubleDCT_1D;
import com.org.jtransforms.dht.DoubleDHT_1D;
import com.org.jtransforms.dst.DoubleDST_1D;
...
public class DenseDoubleMatrix1D extends DoubleMatrix1D {
    ...
    public DoubleMatrix1D assign(final com.cern.colt.function.tdouble.DoubleFunction function) {
        final double multiplicator;
        if (function instanceof com.cern.jet.math.tdouble.DoubleMult) {
            // x[i] = mult*x[i]
            multiplicator = ((com.cern.jet.math.tdouble.DoubleMult) function).multiplicator;
            if (multiplicator == 1) {
                return this;
            }
        } else {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        } else if (function instanceof com.cern.jet.math.tdouble.DoublePlusMultFirst) {
                            double multiplicator = ((com.cern.jet.math.tdouble.DoublePlusMultFirst) function).multiplicator;
                            if (multiplicator == 0) {
                                // x[i] = 0*x[i] + y[i]
                                for (int k = firstIdx; k < lastIdx; k++) {
                                    elements[idx] = elementsOther[idxOther];
