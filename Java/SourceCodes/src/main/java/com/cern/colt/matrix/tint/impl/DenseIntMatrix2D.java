...
public class DenseIntMatrix2D extends IntMatrix2D {
    ...
    public IntMatrix2D assign(final com.cern.colt.function.tint.IntFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tint.IntMult) { // x[i] =
                // mult*x[i]
                int multiplicator = ((com.cern.jet.math.tint.IntMult) function).multiplicator;
                if (multiplicator == 1)
                    return this;
                if (multiplicator == 0)
                    return assign(0);
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function instanceof com.cern.jet.math.tint.IntMult) {
                            // x[i] = mult*x[i]
                            int multiplicator = ((com.cern.jet.math.tint.IntMult) function).multiplicator;
                            if (multiplicator == 1)
                                return;
    ...
    public IntMatrix2D assign(final com.cern.colt.function.tint.IntProcedure cond,
            final com.cern.colt.function.tint.IntFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public IntMatrix2D assign(final IntMatrix2D y, final com.cern.colt.function.tint.IntIntFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tint.IntPlusMultSecond) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function == com.cern.jet.math.tint.IntFunctions.mult) {
                            // x[i] = x[i]*y[i]
                        ...
                        } else if (function == com.cern.jet.math.tint.IntFunctions.div) {
                            // x[i] = x[i] / y[i]
                            idx = zero + firstRow * rowStride;
                            idxOther = zeroOther + firstRow * rowStrideOther;
                            for (int r = firstRow; r < lastRow; r++) {
