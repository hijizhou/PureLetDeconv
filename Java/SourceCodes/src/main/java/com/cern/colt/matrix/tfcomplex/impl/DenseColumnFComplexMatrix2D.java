...
import com.cern.colt.matrix.tfloat.impl.DenseColumnFloatMatrix2D;
...
public class DenseColumnFComplexMatrix2D extends FComplexMatrix2D {
    ...
    public float[] aggregate(final com.cern.colt.function.tfcomplex.FComplexFComplexFComplexFunction aggr,
            final com.cern.colt.function.tfcomplex.FComplexFComplexFunction f) {
        float[] b = new float[2];
        if (size() == 0) {
            b[0] = Float.NaN;
            b[1] = Float.NaN;
    ...
    public float[] aggregate(final FComplexMatrix2D other,
            final com.cern.colt.function.tfcomplex.FComplexFComplexFComplexFunction aggr,
            final com.cern.colt.function.tfcomplex.FComplexFComplexFComplexFunction f) {
    ...
    public FComplexMatrix2D assign(final com.cern.colt.function.tfcomplex.FComplexFComplexFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tfcomplex.FComplexMult) {
                float[] multiplicator = ((com.cern.jet.math.tfcomplex.FComplexMult) function).multiplicator;
    ...
    public FComplexMatrix2D assign(final com.cern.colt.function.tfcomplex.FComplexProcedure cond,
            final com.cern.colt.function.tfcomplex.FComplexFComplexFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
    ...
    public FComplexMatrix2D assign(final com.cern.colt.function.tfcomplex.FComplexProcedure cond, final float[] value) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
    ...
    public FComplexMatrix2D assign(final com.cern.colt.function.tfcomplex.FComplexRealFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, columns);
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        int idx = zero + firstColumn * columnStride;
                        float[] tmp = new float[2];
                        if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.abs) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, r = 0; r < rows; r++) {
                                    tmp[0] = elements[i];
                                    tmp[1] = elements[i + 1];
    ...
    public FComplexMatrix2D assign(final FComplexMatrix2D y,
            final com.cern.colt.function.tfcomplex.FComplexFComplexFComplexFunction function) {
        // overriden for performance only
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.mult) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
                        ...
                        } else if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.multConjFirst) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
                        ...
                        } else if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.multConjSecond) {
                            for (int c = firstColumn; c < lastColumn; c++) {
                                for (int i = idx, j = idxOther, r = 0; r < rows; r++) {
                                    tmp1[0] = elements[i];
                                    tmp1[1] = elements[i + 1];
