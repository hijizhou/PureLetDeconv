...
import com.cern.colt.matrix.tfloat.FloatFactory3D;
 ...
 * 
 */
public class FloatSorting extends com.cern.colt.PersistentObject {
    /**
     * 
     */
     ...
     */
    public FloatMatrix1D sort(final FloatMatrix1D vector, final com.cern.colt.function.tfloat.FloatComparator c) {
     ...
     * @return sorted indexes
     */
    public int[] sortIndex(final FloatMatrix1D vector, final com.cern.colt.function.tfloat.FloatComparator c) {
        int[] indexes = new int[(int) vector.size()]; // row indexes to reorder
        // instead of matrix itself
        for (int i = indexes.length; --i >= 0;)
            indexes[i] = i;
     ...
     * <td align="left" valign="top"> <tt>aggregates=<br>
     com.hep.aida.bin.BinFunctions1D.sum<br>
     ==></tt></td>
     * <td valign="top">
     * <p>
     * <tt>4 x 2 matrix:<br>
     ...
     * FloatMatrix2D sorted = quickSort(matrix, com.hep.aida.bin.BinFunctions1D.median);
     * //FloatMatrix2D sorted = quickSort(matrix,com.hep.aida.bin.BinFunctions1D.sumOfLogarithms);
     * 
     * // THE SLOW VERSION (takes some 300 secs)
     * FloatMatrix1DComparator comparator = new FloatMatrix1DComparator() {
     *     public int compare(FloatMatrix1D x, FloatMatrix1D y) {
     *         float a = com.cern.colt.matrix.floatalgo.Statistic.bin(x).median();
     *         float b = com.cern.colt.matrix.floatalgo.Statistic.bin(y).median();
     *         // float a = x.aggregate(F.plus,F.log);
     *         // float b = y.aggregate(F.plus,F.log);
     *         return a &lt; b ? -1 : a == b ? 0 : 1;
     *     }
     ...
     *         matrix is left unaffected.</b>
     */
    public FloatMatrix2D sort(FloatMatrix2D matrix, com.hep.aida.tfloat.bin.FloatBinFunction1D aggregate) {
        ...
        com.hep.aida.tfloat.bin.FloatBinFunction1D[] func = { aggregate };
    ...
    public static void zdemo3() {
        ...
        com.cern.colt.function.tfloat.FloatComparator comp = new com.cern.colt.function.tfloat.FloatComparator() {
            public int compare(float a, float b) {
        ...
        sorted.assign(com.cern.jet.math.tfloat.FloatFunctions.sin);
        /*
         * sorted.assign( new com.cern.colt.function.FloatFunction() { public
    ...
    protected static void zdemo4() {
        ...
        matrix1.assign(matrix2, com.cern.jet.math.tfloat.FloatFunctions.pow);

        /*
         * matrix1.assign(matrix2, new com.cern.colt.function.FloatFloatFunction() {
    ...
    public static void zdemo5(int rows, int columns, boolean print) {
        ...
        final com.cern.jet.math.tfloat.FloatFunctions F = com.cern.jet.math.tfloat.FloatFunctions.functions;
        FloatMatrix2D A = FloatFactory2D.dense.make(rows, columns);
        A.assign(new com.cern.jet.random.tfloat.engine.FRand()); // initialize randomly
        timer.stop().display();
        ...
        timer.reset().start();
        // THE QUICK VERSION (takes some 10 secs)
        A = sort.sort(A, com.hep.aida.tfloat.bin.FloatBinFunctions1D.median);
        // A = sort.sort(A,com.hep.aida.bin.BinFunctions1D.sumLog);
        timer.stop().display();
        ...
        if (print) {
            ...
            com.hep.aida.tfloat.bin.FloatBinFunction1D[] funs = { com.hep.aida.tfloat.bin.FloatBinFunctions1D.median,
                    com.hep.aida.tfloat.bin.FloatBinFunctions1D.sumLog,
                    com.hep.aida.tfloat.bin.FloatBinFunctions1D.geometricMean };
            ...
            System.out.println("first part of sorted result = \n"
                    + new FloatFormatter("%G").toTitleString(A.viewPart(0, 0, r, columns),
        ...
        FloatMatrix1DComparator fun = new FloatMatrix1DComparator() {
            public int compare(FloatMatrix1D x, FloatMatrix1D y) {
                ...
                float b = FloatStatistic.bin(y).median();
                // float a = x.aggregate(F.plus,F.log);
                // float b = y.aggregate(F.plus,F.log);
                return a < b ? -1 : (a == b) ? 0 : 1;
            }
    ...
    public static void zdemo8(int size) {
        ...
        final com.cern.jet.math.tfloat.FloatFunctions F = com.cern.jet.math.tfloat.FloatFunctions.functions;
        FloatMatrix1D A = com.cern.colt.matrix.tfloat.FloatFactory1D.dense.random(size);
        System.out.print("now quick sorting... ");
        com.cern.colt.Timer timer = new com.cern.colt.Timer().start();
        quickSort.sort(A);
        timer.stop().display();
    ...
    public static void main(String[] args) {
        com.edu.emory.mathcs.utils.pc.ConcurrencyUtils.setNumberOfThreads(2);
        zdemo8(10000000);
        System.exit(0);
    }
}
