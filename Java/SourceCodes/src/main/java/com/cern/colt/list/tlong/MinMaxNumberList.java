...
import com.cern.colt.matrix.tbit.BitVector;
import com.cern.colt.matrix.tbit.QuickBitVector;
 ...
 * implemented with a <tt>com.cern.colt.bitvector.BitVector</tt>. First see the <a
...
public class MinMaxNumberList extends AbstractLongList {
    ...
    public static int bitsPerElement(long minimum, long maximum) {
        ...
        if (1 + maximum - minimum > 0) {
            bits = (int) Math.round(Math.ceil(com.cern.jet.math.tdouble.DoubleArithmetic.log(2, 1 + maximum - minimum)));
        } else {
