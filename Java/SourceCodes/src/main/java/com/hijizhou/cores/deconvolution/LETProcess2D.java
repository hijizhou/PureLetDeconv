...
public class LETProcess2D implements Callable<LETProcess2D> {
	...
	public LETProcess2D call() throws Exception {
		...
		DComplexMatrix2D decEle = new DenseDComplexMatrix2D(width, height);
		DComplexMatrix2D decEleH = new DenseDComplexMatrix2D(width, height);
		DComplexMatrix2D decEleConj = new DenseDComplexMatrix2D(width, height);
		DComplexMatrix2D recEle = new DenseDComplexMatrix2D(width, height);

		AbstractMatrix2D di = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D diReal = new DenseDoubleMatrix2D(width, height);
		AbstractMatrix2D di2 = new DenseDComplexMatrix2D(width, height);
		DComplexMatrix2D Di2 = new DenseDComplexMatrix2D(width, height);

		DComplexMatrix2D aux_w = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D w = new DenseDoubleMatrix2D(width, height);
		DComplexMatrix2D aux_v = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D v = new DenseDoubleMatrix2D(width, height);

		DoubleMatrix2D tp = new DenseDoubleMatrix2D(width, height);
		DoubleMatrix2D betav = new DenseDoubleMatrix2D(width, height);
		DoubleMatrix2D s = new DenseDoubleMatrix2D(width, height);
		DoubleMatrix2D t = new DenseDoubleMatrix2D(width, height);

		DComplexMatrix2D auxrt = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D rt = new DenseDoubleMatrix2D(width, height);

		DComplexMatrix2D DiRt = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D auxDiRt = new DenseDoubleMatrix2D(width, height);

		DComplexMatrix2D auxW = new DenseDComplexMatrix2D(width, height);
		DoubleMatrix2D w1 = new DenseDoubleMatrix2D(width, height);
		DoubleMatrix2D w2 = new DenseDoubleMatrix2D(width, height);
		DoubleMatrix2D ww = new DenseDoubleMatrix2D(width, height);

		wf.fft_wfilters2D(width, height, oIndex, jIndex + 1);

		decEle = PSFUtil.getDecMatrix2D(wf);


		// Di = D.*Hli;
		 decEleH = decEle.copy();
		decEleH.assign(Hli, DComplexFunctions.mult);
		 decEleConj = decEle.copy();
		decEleConj.assign(DComplexFunctions.conj);
		 recEle = decEleConj.copy();
		recEle.assign(DComplexFunctions.div(Math.pow(4, jIndex + 1)));

		 di = decEleH.copy();
		((DenseDComplexMatrix2D) di).ifft2(true);
		 diReal = ((DenseDComplexMatrix2D) di).getRealPart();
		 di2 = diReal.copy();
		((DenseDoubleMatrix2D) di2).assign(DoubleFunctions.square);
		 Di2 = ((DenseDoubleMatrix2D) di2).getFft2();
		...
		((DenseDComplexMatrix2D) auxrt).ifft2(true);
		 rt = auxrt.getRealPart();

		 auxDiRt = rt.copy();
		auxDiRt.assign(diReal, DoubleFunctions.mult);
		 DiRt = ((DenseDoubleMatrix2D) auxDiRt).getFft2();
		auxDiRt = rt.copy();
		auxDiRt.assign((DenseDoubleMatrix2D) di2, DoubleFunctions.mult);
