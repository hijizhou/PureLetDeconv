	...
	*/
	...
	public void rescale() {
		...
		for(int z=0; z<nz; z++) {
			slice = (byte[])data[z];
			for(int k=0; k<nxy; k++) {
				slice[k] = (byte)(a*((slice[k] & 0xFF)-minImage));
			}
		}
	}
	...
	*/
	...
	public void rescale(double minLevel, double maxLevel) {
		...
		for(int z=0; z<nz; z++) {
			slice = (byte[])data[z];
			for(int k=0; k<nxy; k++) {
				slice[k] = (byte)(a*((slice[k] & 0xFF)-minImage) + minLevel);
			}
		}
	}
	...
	*/
	...
	public void rescaleCenter(double minLevel, double maxLevel) {
		...
		for(int z=0; z<nz; z++) {
			slice = (byte[])data[z];
			for(int k=0; k<nxy; k++) {
				slice[k] = (byte)(a*((slice[k] & 0xFF)-minImage) + center);
			}
		}
	}
