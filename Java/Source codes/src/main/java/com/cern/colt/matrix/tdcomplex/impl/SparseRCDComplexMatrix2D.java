...
public class SparseRCDComplexMatrix2D extends WrapperDComplexMatrix2D {
    ...
    public DComplexMatrix2D assign(final DComplexMatrix2D y,
        ...
        checkShape(y);
        if ((y instanceof SparseRCDComplexMatrix2D) && (function == DComplexFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            int ncol = columns;
            int nzmax = cnz;
            if (function == DComplexFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == DComplexFunctions.mult) { // x[i] = x[i] * y[i]
            double[] elem = new double[2];
        ...
        if (function == DComplexFunctions.div) { // x[i] = x[i] / y[i]
            double[] elem = new double[2];
            for (int i = 0; i < rows; i++) {
                int high = rowPointers[i + 1];
                for (int k = rowPointers[i]; k < high; k++) {
