...
public class DenseLongMatrix1D extends LongMatrix1D {
    ...
    public LongMatrix1D assign(final com.cern.colt.function.tlong.LongFunction function) {
        final long multiplicator;
        if (function instanceof com.cern.jet.math.tlong.LongMult) {
            // x[i] = mult*x[i]
            multiplicator = ((com.cern.jet.math.tlong.LongMult) function).multiplicator;
            if (multiplicator == 1) {
                return this;
            }
        } else {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        } else if (function instanceof com.cern.jet.math.tlong.LongPlusMultFirst) {
                            long multiplicator = ((com.cern.jet.math.tlong.LongPlusMultFirst) function).multiplicator;
                            if (multiplicator == 0) {
                                // x[i] = 0*x[i] + y[i]
                                for (int k = firstIdx; k < lastIdx; k++) {
                                    elements[idx] = elemsOther[idxOther];
