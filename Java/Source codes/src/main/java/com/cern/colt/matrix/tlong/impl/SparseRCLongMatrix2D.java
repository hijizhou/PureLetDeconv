...
import com.cern.jet.math.tlong.LongFunctions;
 ...
 * {@link LongFunctions#plusMultSecond} as argument.
 * 
 * 
...
public class SparseRCLongMatrix2D extends WrapperLongMatrix2D {
    ...
    public LongMatrix2D assign(final LongMatrix2D y, com.cern.colt.function.tlong.LongLongFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCLongMatrix2D) && (function == LongFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            if (function == LongFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == LongFunctions.mult) { // x[i] = x[i] * y[i]
            for (int i = rows; --i >= 0;) {
        ...
        if (function == LongFunctions.div) { // x[i] = x[i] / y[i]
