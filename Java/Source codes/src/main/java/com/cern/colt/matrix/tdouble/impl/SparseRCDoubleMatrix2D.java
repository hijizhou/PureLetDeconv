 ...
 * {@link DoubleFunctions#plusMultSecond} as argument.
 * 
 * 
...
public class SparseRCDoubleMatrix2D extends WrapperDoubleMatrix2D {
    ...
    public DoubleMatrix2D assign(final DoubleMatrix2D y, com.cern.colt.function.tdouble.DoubleDoubleFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCDoubleMatrix2D) && (function == DoubleFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            if (function == DoubleFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == DoubleFunctions.div) { // x[i] = x[i] / y[i]
    ...
    public synchronized double getQuick(int row, int column) {
        //        int k = com.cern.colt.Sorting.binarySearchFromTo(columnIndexes, column, rowPointers[row], rowPointers[row + 1] - 1);
    ...
    public DoubleMatrix2D zMult(DoubleMatrix2D B, DoubleMatrix2D C, final double alpha, double beta,
        ...
        if ((B instanceof DenseDoubleMatrix2D) && (C instanceof DenseDoubleMatrix2D)) {
            ...
            for (int ii = 0; ii < rowsA; ii++) {
                ...
                //                int[] indexes = com.cern.colt.matrix.tint.algo.IntSorting.quickSort.sortIndex(columnIndexesCPart);
