...
import com.org.jtransforms.dct.FloatDCT_1D;
import com.org.jtransforms.dht.FloatDHT_1D;
import com.org.jtransforms.dst.FloatDST_1D;
import com.org.jtransforms.fft.FloatFFT_1D;
...
public class DenseFloatMatrix1D extends FloatMatrix1D {
    ...
    public FloatMatrix1D assign(final com.cern.colt.function.tfloat.FloatFunction function) {
        final float multiplicator;
        if (function instanceof com.cern.jet.math.tfloat.FloatMult) {
            // x[i] = mult*x[i]
            multiplicator = ((com.cern.jet.math.tfloat.FloatMult) function).multiplicator;
            if (multiplicator == 1) {
                return this;
            }
        } else {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    public void run() {
                        ...
                        } else if (function instanceof com.cern.jet.math.tfloat.FloatPlusMultFirst) {
                            float multiplicator = ((com.cern.jet.math.tfloat.FloatPlusMultFirst) function).multiplicator;
                            if (multiplicator == 0) {
                                // x[i] = 0*x[i] + y[i]
                                for (int k = firstIdx; k < lastIdx; k++) {
                                    elements[idx] = elementsOther[idxOther];
