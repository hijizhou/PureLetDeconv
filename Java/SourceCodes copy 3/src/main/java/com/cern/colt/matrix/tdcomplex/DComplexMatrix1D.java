...
public abstract class DComplexMatrix1D extends AbstractMatrix1D {
     ...
     * @see com.cern.jet.math.tdcomplex.DComplexFunctions
     */
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexDComplexFunction f) {
        int size = (int) size();
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
     ...
     * @see com.cern.jet.math.tdcomplex.DComplexFunctions
     */
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexRealFunction f) {
        int size = (int) size();
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
    ...
    public boolean equals(Object obj) {
        ...
        return com.cern.colt.matrix.tdcomplex.algo.DComplexProperty.DEFAULT.equals(this, (DComplexMatrix1D) obj);
    }
     ...
     * @return the new view.
     */
    public DComplexMatrix1D viewSelection(com.cern.colt.function.tdcomplex.DComplexProcedure condition) {
        int size = (int) size();
