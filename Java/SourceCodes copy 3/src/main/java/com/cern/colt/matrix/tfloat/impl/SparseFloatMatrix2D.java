...
import com.cern.colt.map.tfloat.AbstractLongFloatMap;
import com.cern.colt.map.tfloat.OpenLongFloatHashMap;
 ...
 * {@link OpenLongFloatHashMap}, which is a compact and
...
public class SparseFloatMatrix2D extends FloatMatrix2D {
     ...
     * to memory usage see {@link OpenLongFloatHashMap}.
     * 
     * @param rows
     *            the number of rows the matrix shall have.
     * @param columns
    ...
    public FloatMatrix2D assign(com.cern.colt.function.tfloat.FloatFunction function) {
        if (this.isNoView && function instanceof com.cern.jet.math.tfloat.FloatMult) { // x[i] = mult*x[i]
    ...
    public FloatMatrix2D assign(final FloatMatrix2D y, com.cern.colt.function.tfloat.FloatFloatFunction function) {
        ...
        } else if (function == com.cern.jet.math.tfloat.FloatFunctions.mult) { // x[i] = x[i] * y[i]
            this.elements.forEachPair(new com.cern.colt.function.tfloat.LongFloatProcedure() {
        ...
        } else if (function == com.cern.jet.math.tfloat.FloatFunctions.div) { // x[i] = x[i] /  y[i]
            this.elements.forEachPair(new com.cern.colt.function.tfloat.LongFloatProcedure() {
                public boolean apply(long key, float value) {
                    int i = (int) (key / columns);
                    int j = (int) (key % columns);
    ...
    public SparseFloatMatrix2D assign(final int[] rowIndexes, final int[] columnIndexes, final float value,
            final com.cern.colt.function.tfloat.FloatFloatFunction function) {
        int size = rowIndexes.length;
        if (function == com.cern.jet.math.tfloat.FloatFunctions.plus) { // x[i] = x[i] + y[i]
    ...
    public FloatMatrix1D zMult(FloatMatrix1D y, FloatMatrix1D z, final float alpha, float beta, final boolean transposeA) {
        ...
        if (!ignore)
            z.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(beta));
