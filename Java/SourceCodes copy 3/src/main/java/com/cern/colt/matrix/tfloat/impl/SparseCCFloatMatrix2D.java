 ...
 */
package com.cern.colt.matrix.tfloat.impl;
...
import com.edu.emory.mathcs.csparsej.tfloat.Scs_add;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_cumsum;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_dropzeros;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_dupl;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_multiply;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_transpose;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_util;
import com.edu.emory.mathcs.csparsej.tfloat.Scs_common.Scs;
 ...
 * A.forEachNonZero(new com.cern.colt.function.IntIntFloatFunction() {
 ...
 * B.forEachNonZero(new com.cern.colt.function.IntIntFloatFunction() {
 *     public float apply(int row, int column, float value) {
 ...
 * </table>
 * Method
 * {@link #assign(FloatMatrix2D,com.cern.colt.function.tfloat.FloatFloatFunction)}
 * does just that if you supply
 * {@link com.cern.jet.math.tfloat.FloatFunctions#plusMultSecond} as argument.
 * 
 * 
...
public class SparseCCFloatMatrix2D extends WrapperFloatMatrix2D {
    ...
    public FloatMatrix2D assign(final com.cern.colt.function.tfloat.FloatFunction function) {
        if (function instanceof com.cern.jet.math.tfloat.FloatMult) { // x[i] = mult*x[i]
            final float alpha = ((com.cern.jet.math.tfloat.FloatMult) function).multiplicator;
            if (alpha == 1)
                return this;
            if (alpha == 0)
                return assign(0);
            ...
            forEachNonZero(new com.cern.colt.function.tfloat.IntIntFloatFunction() {
    ...
    public FloatMatrix2D assign(FloatMatrix2D source) {
        ...
        if (source instanceof SparseCCFloatMatrix2D) {
            ...
            assign(0);
            source.forEachNonZero(new com.cern.colt.function.tfloat.IntIntFloatFunction() {
                public float apply(int i, int j, float value) {
                    setQuick(i, j, value);
                    return value;
                }
    ...
    public FloatMatrix2D assign(final FloatMatrix2D y, com.cern.colt.function.tfloat.FloatFloatFunction function) {
        checkShape(y);

        if ((y instanceof SparseCCFloatMatrix2D) && (function == com.cern.jet.math.tfloat.FloatFunctions.plus)) { // x[i] = x[i] + y[i]
            SparseCCFloatMatrix2D yy = (SparseCCFloatMatrix2D) y;
            scs = Scs_add.cs_add(scs, yy.scs, 1, 1);
            return this;
        }

        if (function instanceof com.cern.jet.math.tfloat.FloatPlusMultSecond) { // x[i] = x[i] + alpha*y[i]
            final float alpha = ((com.cern.jet.math.tfloat.FloatPlusMultSecond) function).multiplicator;
            if (alpha == 0)
                return this; // nothing to do
            y.forEachNonZero(new com.cern.colt.function.tfloat.IntIntFloatFunction() {
                public float apply(int i, int j, float value) {
        ...
        if (function instanceof com.cern.jet.math.tfloat.FloatPlusMultFirst) { // x[i] = alpha*x[i] + y[i]
            final float alpha = ((com.cern.jet.math.tfloat.FloatPlusMultFirst) function).multiplicator;
            if (alpha == 0)
                return assign(y);
            y.forEachNonZero(new com.cern.colt.function.tfloat.IntIntFloatFunction() {
                public float apply(int i, int j, float value) {
        ...
        if (function == com.cern.jet.math.tfloat.FloatFunctions.mult) { // x[i] = x[i] * y[i]
        ...
        if (function == com.cern.jet.math.tfloat.FloatFunctions.div) { // x[i] = x[i] / y[i]
            final int[] rowIndexesA = scs.i;
            final int[] columnPointersA = scs.p;
            final float[] valuesA = scs.x;
    ...
    public FloatMatrix2D forEachNonZero(final com.cern.colt.function.tfloat.IntIntFloatFunction function) {
        final int[] rowIndexesA = scs.i;
        final int[] columnPointersA = scs.p;
        final float[] valuesA = scs.x;
    ...
    public FloatMatrix1D zMult(FloatMatrix1D y, FloatMatrix1D z, final float alpha, final float beta,
        ...
        if (!transposeA) {
            if ((!ignore) && (beta / alpha != 1.0)) {
                z.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(beta / alpha));
            }
            ...
            if (alpha != 1.0) {
                z.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(alpha));
            }
        } else {
            if ((nthreads > 1) && (cardinality() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
                Future<?>[] futures = new Future[nthreads];
    ...
    public FloatMatrix2D zMult(FloatMatrix2D B, FloatMatrix2D C, final float alpha, float beta,
        ...
        if (!ignore && beta != 1.0) {
            C.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(beta));
        }
        ...
        if ((B instanceof DenseFloatMatrix2D) && (C instanceof DenseFloatMatrix2D)) {
            ...
            if (alpha != 1.0) {
                C.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(alpha));
            }
            ...
            if (alpha != 1.0) {
                CC.assign(com.cern.jet.math.tfloat.FloatFunctions.mult(alpha));
            }
        } else {
            if (transposeB) {
                B = B.viewDice();
            ...
            final com.cern.jet.math.tfloat.FloatPlusMultSecond fun = com.cern.jet.math.tfloat.FloatPlusMultSecond.plusMult(0);
