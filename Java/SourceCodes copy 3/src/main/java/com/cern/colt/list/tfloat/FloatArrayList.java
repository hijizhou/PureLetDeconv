...
public class FloatArrayList extends AbstractFloatList {
    ...
    public boolean removeAll(AbstractFloatList other) {
        ...
        float N = other.size();
        float M = mySize;
        if ((N + M) * com.cern.jet.math.tfloat.FloatArithmetic.log2(N) < M * N) {
    ...
    public void shuffleFromTo(int from, int to) {
        ...
        com.cern.jet.random.tfloat.FloatUniform gen = new com.cern.jet.random.tfloat.FloatUniform(
                new com.cern.jet.random.tfloat.engine.FRand(new java.util.Date()));
        float tmpElement;
        float[] theElements = elements;
        int random;
        for (int i = from; i < to; i++) {
