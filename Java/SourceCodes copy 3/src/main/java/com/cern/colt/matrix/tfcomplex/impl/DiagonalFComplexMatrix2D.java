...
public class DiagonalFComplexMatrix2D extends WrapperFComplexMatrix2D {
                ...
                for (int j = 0; j < nthreads; j++) {
                    ...
                    futures[j] = ConcurrencyUtils.submit(new Runnable() {
                        ...
                        public void run() {
                            ...
                            } else if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.mult) { // x[i] = x[i] * y[i]
                                float[] elem = new float[2];
                            ...
                            } else if (function == com.cern.jet.math.tfcomplex.FComplexFunctions.div) { // x[i] = x[i] /  y[i]
                                float[] elem = new float[2];
                                float[] otherElem = new float[2];
                                for (int j = firstIdx; j < lastIdx; j++) {
                                    otherElem[0] = otherElements[2 * j];
    ...
    public boolean equals(float[] value) {
        float epsilon = com.cern.colt.matrix.tfcomplex.algo.FComplexProperty.DEFAULT.tolerance();
        float[] x = new float[2];
