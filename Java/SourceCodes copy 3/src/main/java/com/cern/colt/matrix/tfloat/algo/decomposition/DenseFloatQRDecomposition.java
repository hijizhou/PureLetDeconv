...
public class DenseFloatQRDecomposition implements java.io.Serializable {
    ...
    public DenseFloatQRDecomposition(FloatMatrix2D A) {
        ...
        Rdiag = A.like1D(n);
        // Rdiag = new float[n];
        com.cern.colt.function.tfloat.FloatFloatFunction hypot = DenseFloatAlgebra.hypotFunction();
    ...
    public String toString() {
        ...
        buf.append("\n\npseudo inverse(A) = ");
        try {
            buf.append(String.valueOf(this.solve(com.cern.colt.matrix.tfloat.FloatFactory2D.dense.identity(QR.rows()))));
        } catch (IllegalArgumentException exc) {
            buf.append(unknown + exc.getMessage());
        }
