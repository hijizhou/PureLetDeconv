 ...
 * {@link FloatFunctions#plusMultSecond} as argument.
 * 
 * 
...
public class SparseRCFloatMatrix2D extends WrapperFloatMatrix2D {
    ...
    public FloatMatrix2D assign(final FloatMatrix2D y, com.cern.colt.function.tfloat.FloatFloatFunction function) {
        checkShape(y);
        if ((y instanceof SparseRCFloatMatrix2D) && (function == FloatFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            if (function == FloatFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == FloatFunctions.mult) { // x[i] = x[i] * y[i]
            for (int i = rows; --i >= 0;) {
        ...
        if (function == FloatFunctions.div) { // x[i] = x[i] / y[i]
