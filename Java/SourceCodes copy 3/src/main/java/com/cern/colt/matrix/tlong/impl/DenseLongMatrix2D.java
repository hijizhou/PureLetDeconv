...
public class DenseLongMatrix2D extends LongMatrix2D {
    ...
    public LongMatrix2D assign(final com.cern.colt.function.tlong.LongFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tlong.LongMult) { // x[i] =
                // mult*x[i]
                long multiplicator = ((com.cern.jet.math.tlong.LongMult) function).multiplicator;
                if (multiplicator == 1)
                    return this;
                if (multiplicator == 0)
                    return assign(0);
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function instanceof com.cern.jet.math.tlong.LongMult) {
                            // x[i] = mult*x[i]
                            long multiplicator = ((com.cern.jet.math.tlong.LongMult) function).multiplicator;
                            if (multiplicator == 1)
                                return;
    ...
    public LongMatrix2D assign(final com.cern.colt.function.tlong.LongProcedure cond,
            final com.cern.colt.function.tlong.LongFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public LongMatrix2D assign(final LongMatrix2D y, final com.cern.colt.function.tlong.LongLongFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tlong.LongPlusMultSecond) {
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function == com.cern.jet.math.tlong.LongFunctions.mult) {
                            // x[i] = x[i]*y[i]
                        ...
                        } else if (function == com.cern.jet.math.tlong.LongFunctions.div) {
                            // x[i] = x[i] / y[i]
                            idx = zero + firstRow * rowStride;
                            idxOther = zeroOther + firstRow * rowStrideOther;
                            for (int r = firstRow; r < lastRow; r++) {
