...
public class DenseDComplexMatrix1D extends DComplexMatrix1D {
    ...
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexDComplexFunction function) {
    ...
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexProcedure cond,
    ...
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexProcedure cond, final double[] value) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
            nthreads = Math.min(nthreads, size);
            Future<?>[] futures = new Future[nthreads];
    ...
    public DComplexMatrix1D assign(final com.cern.colt.function.tdcomplex.DComplexRealFunction function) {
        if (this.elements == null)
            throw new InternalError();
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
