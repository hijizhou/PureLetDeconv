...
import com.org.jtransforms.dct.FloatDCT_2D;
import com.org.jtransforms.dht.FloatDHT_2D;
import com.org.jtransforms.dst.FloatDST_2D;
import com.org.jtransforms.fft.FloatFFT_2D;
...
public class DenseFloatMatrix2D extends FloatMatrix2D {
    ...
    public float aggregate(final com.cern.colt.function.tfloat.FloatFloatFunction aggr,
            final com.cern.colt.function.tfloat.FloatFunction f) {
        if (size() == 0)
            return Float.NaN;
    ...
    public float aggregate(final com.cern.colt.function.tfloat.FloatFloatFunction aggr,
            final com.cern.colt.function.tfloat.FloatFunction f, final com.cern.colt.function.tfloat.FloatProcedure cond) {
        if (size() == 0)
            return Float.NaN;
        final int zero = (int) index(0, 0);
        float a = 0;
    ...
    public float aggregate(final com.cern.colt.function.tfloat.FloatFloatFunction aggr,
            final com.cern.colt.function.tfloat.FloatFunction f, final IntArrayList rowList, final IntArrayList columnList) {
        if (size() == 0)
            return Float.NaN;
        final int zero = (int) index(0, 0);
    ...
    public float aggregate(final FloatMatrix2D other, final com.cern.colt.function.tfloat.FloatFloatFunction aggr,
            final com.cern.colt.function.tfloat.FloatFloatFunction f) {
    ...
    public FloatMatrix2D assign(final com.cern.colt.function.tfloat.FloatFunction function) {
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tfloat.FloatMult) { // x[i] =
                // mult*x[i]
                float multiplicator = ((com.cern.jet.math.tfloat.FloatMult) function).multiplicator;
                if (multiplicator == 1)
                    return this;
                if (multiplicator == 0)
                    return assign(0);
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function instanceof com.cern.jet.math.tfloat.FloatMult) {
                            // x[i] = mult*x[i]
                            float multiplicator = ((com.cern.jet.math.tfloat.FloatMult) function).multiplicator;
                            if (multiplicator == 1)
                                return;
    ...
    public FloatMatrix2D assign(final com.cern.colt.function.tfloat.FloatProcedure cond,
            final com.cern.colt.function.tfloat.FloatFunction function) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public FloatMatrix2D assign(final com.cern.colt.function.tfloat.FloatProcedure cond, final float value) {
        final int zero = (int) index(0, 0);
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, rows);
    ...
    public FloatMatrix2D assign(final FloatMatrix2D y, final com.cern.colt.function.tfloat.FloatFloatFunction function) {
        // overriden for performance only
        ...
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            if (function instanceof com.cern.jet.math.tfloat.FloatPlusMultSecond) {
                float multiplicator = ((com.cern.jet.math.tfloat.FloatPlusMultSecond) function).multiplicator;
            ...
            for (int j = 0; j < nthreads; j++) {
                ...
                futures[j] = ConcurrencyUtils.submit(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (function == com.cern.jet.math.tfloat.FloatFunctions.mult) {
                            // x[i] = x[i]*y[i]
                        ...
                        } else if (function == com.cern.jet.math.tfloat.FloatFunctions.div) {
                            // x[i] = x[i] / y[i]
                            idx = zero + firstRow * rowStride;
                            idxOther = zeroOther + firstRow * rowStrideOther;
                            for (int r = firstRow; r < lastRow; r++) {
                        ...
                        } else if (function instanceof com.cern.jet.math.tfloat.FloatPlusMultSecond) {
                            float multiplicator = ((com.cern.jet.math.tfloat.FloatPlusMultSecond) function).multiplicator;
                            if (multiplicator == 1) {
                                // x[i] = x[i] + y[i]
                                idx = zero + firstRow * rowStride;
    ...
    public void zAssign8Neighbors(FloatMatrix2D B, com.cern.colt.function.tfloat.Float9Function function) {
