...
//    private JRadioButton checkMSELET = new JRadioButton("MSE-LET?", false);
    ...
    private JRadioButton checkRunLog = new JRadioButton("Show log?", false);
    //Part-Run
...
//        pnRunResult.place(0, 2, checkRunShowPSF);
        ...
        pnRunButtons.place(0, 2, checkRunLog);
...
//        }
        ...
        MW_PURE_LET2D sl;
        sl = new MW_PURE_LET2D(Input, PSF, noisePar, this.walk, false);
...
//        Input = rescale(Input, 1, 255);
        ...
        Boolean Log = false;
        if(this.checkRunLog.isSelected()){Log = true;}

        if(Log){
            IJ.log("Gaussian PSF size: " + psfsigma);
        }
        MW_PURE_LET2D sl;
        sl = new MW_PURE_LET2D(Input, PSF, noisePar, this.walk, Log);
