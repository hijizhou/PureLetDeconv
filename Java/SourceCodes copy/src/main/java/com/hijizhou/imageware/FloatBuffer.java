	...
	protected FloatBuffer(ImageStack stack, byte channel) {
		...
		if (ip instanceof ByteProcessor) {
			...
			for (int z=0; z<nz; z++) {
				...
				case ImageWare.RED:
					for (int k=0; k<nxy; k++) {
						((float[])data[z])[k] = (pixels[k] & 0xFF0000)>>16;
					}
					break;
				case ImageWare.GREEN:
					for (int k=0; k<nxy; k++) {
						((float[])data[z])[k] = (pixels[k] & 0xFF00)>>8;
					}
					break;
				case ImageWare.BLUE:
					for (int k=0; k<nxy; k++) {
						((float[])data[z])[k] = pixels[k] & 0xFF;
					}
					break;
				default:
					throw_constructor();
	...
	*/
	...
	public void putZ(int x, int y, int z, byte[] buffer) {
		try {
			...
			int leni = buffer.length;
			for (int i=0; i<leni; i++) {
				((float[])data[z])[offset] = buffer[i] & 0xFF;
				z++;
			}
		}
		catch(Exception e) {
	...
	*/
	...
	public void putZ(int x, int y, int z, short[] buffer) {
		try {
			...
			int leni = buffer.length;
			for (int i=0; i<leni; i++) {
				((float[])data[z])[offset] = buffer[i] & 0xFFFF;
				z++;
			}
		}
		catch(Exception e) {
	...
	public void putXZ(int x, int y, int z, byte[][] buffer) {
		try {
			...
			for (int j=0; j<lenj; j++, z++) {
				offset = x + j*nx;
				for (int i=0; i<leni; i++, offset++) {
					((float[])data[z])[offset] = buffer[i][j] & 0xFF;
				}
			}
		}
		catch(Exception e) {
	...
	public void putXZ(int x, int y, int z, short[][] buffer) {
		try {
			...
			for (int j=0; j<lenj; j++, z++) {
				offset = x + j*nx;
				for (int i=0; i<leni; i++, offset++) {
					((float[])data[z])[offset] = buffer[i][j] & 0xFFFF;
				}
			}
		}
		catch(Exception e) {
	...
	public void putXZ(int x, int y, int z, float[][] buffer) {
		try {
			...
			for (int j=0; j<lenj; j++, z++) {
				offset = x + j*nx;
				for (int i=0; i<leni; i++, offset++) {
					((float[])data[z])[offset] = (buffer[i][j]);
				}
			}
		}
		catch(Exception e) {
