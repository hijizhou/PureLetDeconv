	...
	public double getInterpolatedPixel(double x, double y, double z) {
		...
		*/
		...
		switch(getDimension()) {	
			...
			case 3:				
				double v3_000 = (((float[])data[k])[index]);
	...
	public void getBoundedXZ(int x, int y, int z, float[][] buffer) {
		try {
			...
			for (int j=jinf; j<jsup; j++) {
				offset = (x+iinf) + (y)*nx;
				for (int i=iinf; i<isup; i++) {
					buffer[i][j] = (((float[])data[z])[offset]);
					offset++;
				}
				k++;
			}
		...
		try {
			...
			for (int j=0; j<lenj; j++) {
				...
				for (int i=0; i<leni; i++) {
					...
					buffer[i][j] = (((float[])data[zp])[xp + yp]);
				}
			}
		}
		catch(Exception e) {
		...
		try {
			...
			for (int j=0; j<lenj; j++) {
				...
				for (int i=0; i<leni; i++) {
					...
					buffer[i][j] = (((float[])data[zp])[xp + yp*nx]);
				}
			}
		}
		catch(Exception e) {
	...
	*/
	...
	public void putBoundedZ(int x, int y, int z, byte[] buffer) {
		try {
			...
			int isup = (z+leni >= nz ? nz-z : leni);
			for (int i=iinf; i<isup; i++) {
				((float[])data[k])[offset] = buffer[i] & 0xFF;
				k++;
			}
		}
		catch(Exception e) {
	...
	*/
	...
	public void putBoundedZ(int x, int y, int z, short[] buffer) {
		try {
			...
			int isup = (z+leni >= nz ? nz-z : leni);
			for (int i=iinf; i<isup; i++) {
				((float[])data[k])[offset] = buffer[i] & 0xFFFF;
				k++;
			}
		}
		catch(Exception e) {
	...
	public void putBoundedXZ(int x, int y, int z, byte[][] buffer) {
		try {
			...
			for (int j=jinf; j<jsup; j++) {
				offset = (x+iinf) + (y)*nx;
				for (int i=iinf; i<isup; i++) {
					((float[])data[k])[offset] = buffer[i][j] & 0xFF;
					offset++;
				}
				k++;
			}
	...
	public void putBoundedXZ(int x, int y, int z, short[][] buffer) {
		try {
			...
			for (int j=jinf; j<jsup; j++) {
				offset = (x+iinf) + (y)*nx;
				for (int i=iinf; i<isup; i++) {
					((float[])data[k])[offset] = buffer[i][j] & 0xFFFF;
					offset++;
				}
				k++;
			}
	...
	public void putBoundedXZ(int x, int y, int z, float[][] buffer) {
		try {
			...
			for (int j=jinf; j<jsup; j++) {
				offset = (x+iinf) + (y)*nx;
				for (int i=iinf; i<isup; i++) {
					((float[])data[k])[offset] = (buffer[i][j]);
					offset++;
				}
				k++;
			}
