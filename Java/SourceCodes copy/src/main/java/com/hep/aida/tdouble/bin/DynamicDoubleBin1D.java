...
public class DynamicDoubleBin1D extends QuantileDoubleBin1D {
     ...
     * @see com.cern.jet.math.tdouble.DoubleFunctions
     */
    public synchronized double aggregate(com.cern.colt.function.tdouble.DoubleDoubleFunction aggr,
            com.cern.colt.function.tdouble.DoubleFunction f) {
        int s = size();
        if (s == 0)
            return Double.NaN;
     ...
     */
    private synchronized com.cern.colt.map.tdouble.AbstractDoubleIntMap frequencyMap() {
        // com.cern.colt.map.OpenDoubleIntHashMap.hashCollisions = 0;
        // fill a map that collects frequencies
        com.cern.colt.map.tdouble.AbstractDoubleIntMap map = new com.cern.colt.map.tdouble.OpenDoubleIntHashMap();
        ...
        // System.out.println("collisions="+com.cern.colt.map.OpenDoubleIntHashMap.hashCollisions);
    ...
    public synchronized void sample(int n, boolean withReplacement, DoubleRandomEngine randomGenerator,
        ...
        if (!withReplacement) { // without
            ...
            com.cern.jet.random.tdouble.sampling.DoubleRandomSamplingAssistant sampler = new com.cern.jet.random.tdouble.sampling.DoubleRandomSamplingAssistant(
            ...
            com.cern.jet.random.tdouble.DoubleUniform uniform = new com.cern.jet.random.tdouble.DoubleUniform(randomGenerator);
            int s = size();
            for (int i = n; --i >= 0;) {
     ...
     * 	 X.addAllOf(new com.cern.colt.list.DoubleArrayList(v1));
     * 	 Y.addAllOf(new com.cern.colt.list.DoubleArrayList(v2));
    ...
    public synchronized DynamicDoubleBin1D sampleBootstrap(DynamicDoubleBin1D other, int resamples,
            DoubleRandomEngine randomGenerator, DoubleBinBinFunction1D function) {
        if (randomGenerator == null)
            randomGenerator = AbstractDoubleDistribution.makeDefaultGenerator();
        ...
        com.cern.colt.buffer.tdouble.DoubleBuffer buffer1 = sample1.buffered(Math.min(maxCapacity, s1));

        DynamicDoubleBin1D sample2 = new DynamicDoubleBin1D();
        com.cern.colt.buffer.tdouble.DoubleBuffer buffer2 = sample2.buffered(Math.min(maxCapacity, s2));

        DynamicDoubleBin1D bootstrap = new DynamicDoubleBin1D();
        com.cern.colt.buffer.tdouble.DoubleBuffer bootBuffer = bootstrap.buffered(Math.min(maxCapacity, resamples));
