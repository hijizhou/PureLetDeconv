...
public class SparseRCFComplexMatrix2D extends WrapperFComplexMatrix2D {
    ...
    public FComplexMatrix2D assign(final FComplexMatrix2D y,
        ...
        checkShape(y);
        if ((y instanceof SparseRCFComplexMatrix2D) && (function == FComplexFunctions.plus)) { // x[i] = x[i] + y[i]
            ...
            int ncol = columns;
            int nzmax = cnz;
            if (function == FComplexFunctions.plus) { // x[i] = x[i] + y[i]
                int kc = 0;
                rowPointersC[0] = kc;
                int j1, j2;
        ...
        if (function == FComplexFunctions.mult) { // x[i] = x[i] * y[i]
            float[] elem = new float[2];
        ...
        if (function == FComplexFunctions.div) { // x[i] = x[i] / y[i]
            float[] elem = new float[2];
            for (int i = 0; i < rows; i++) {
                int high = rowPointers[i + 1];
                for (int k = rowPointers[i]; k < high; k++) {
